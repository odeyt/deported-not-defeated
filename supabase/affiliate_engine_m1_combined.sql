-- ============================================================
-- M-AFFILIATE1 — COMBINED PRODUCTION ACTIVATION
--
-- This file is GENERATED. Do not hand-edit the middle sections.
-- Regenerate with the script documented in
...
-- ============================================================
-- 12. EXPLICIT READ GRANTS
--
-- Supabase's default privileges normally grant SELECT on new objects in the
-- public schema to anon/authenticated, but that depends on project settings.
-- These grants are stated outright so the anonymous read path is not left to
-- a default: the application falls back to the anon client when no
-- service-role key is configured, and it must keep working.
--
-- RLS still applies to the underlying tables. affiliate_conversions is
-- deliberately excluded — section 7 revokes anonymous access to it.
-- ============================================================
grant select on affiliate_providers                to anon, authenticated;
grant select on affiliate_canonical_categories     to anon, authenticated;
grant select on affiliate_provider_countries       to anon, authenticated;
...
-- ============================================================
-- POST-APPLY VERIFICATION — run these separately, after the commit
-- ============================================================
-- 1. Anonymous must NOT hold operator columns (expect zero rows):
--    select column_name from information_schema.column_privileges
--     where grantee = 'anon' and table_name = 'affiliate_partners'
--       and column_name in ('notes','internal_notes','account_identifier',
--                           'commission_value','commission_notes');
--
-- 2. Anonymous must NOT be able to write clicks (expect zero rows):
--    select privilege_type from information_schema.role_table_grants
--     where grantee = 'anon' and table_name = 'affiliate_clicks';
--
-- 3. At least one administrator (expect >= 1):
--    select count(*) from user_profiles where role = 'admin';
--
-- 4. The three live programs must be untouched (expect wise, safetywing,
--    numeromoney, each still 'approved' with a non-null affiliate_url):
--    select slug, affiliate_status, affiliate_url is not null as has_url
--      from affiliate_partners where affiliate_status = 'approved' order by slug;
--
-- 5. Then, in a private browser window:
--      /resources                 provider cards still render
--      /go/wise                   redirects to the Wise affiliate URL
--      /admin/affiliates          still reachable when signed in
-- ============================================================
